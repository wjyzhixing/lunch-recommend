...
//    }
   ...
   //记录用户行为日志
   let paramsIn = (JSON.stringify(options.params) == "{}" ? options.data : options.params)
   const params = {
     loginChannel: 'pc',
     actionPath: url,
     type: 2,
...
//    const addUserBehaviorHref = process.env.HZDJ_SIGN + '/userLog/addUserBehaviorLog';
   const flag = window.location.origin.indexOf('djhzdjtest') > -1 || window.location.origin.indexOf('localhost') > -1; // 如果是测试环境或者是本地
   if(token != null && token != undefined && url.indexOf('getUserBehaviorLogList') == -1 && !flag){
